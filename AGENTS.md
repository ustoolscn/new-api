...
- Desktop OAuth PKCE integration is documented in `docs/desktop-oauth-pkce.md`: fixed public clients `hi-codex` (`Hi Codex`) and `dreamfactory` (`DreamFactory`), canonical scopes `api_keys:read account:read` (URL-encoded as `api_keys%3Aread%20account%3Aread` and accepted in either order), Authorization Code + PKCE S256 over loopback `127.0.0.1`/`::1`, browser consent at `/oauth/authorize`, API endpoints `/api/oauth/authorize`, `/api/oauth/authorize/decision`, `/api/oauth/token`, `/api/oauth/api-keys`, and `/api/oauth/account`, 10-minute authorization requests, 2-minute one-time codes, and 30-day access tokens without refresh tokens. OAuth access tokens are dedicated `oa-...` bearer login credentials (not API keys and never `model.Token` rows); `/api/oauth/api-keys` returns all non-deleted API keys with complete `sk-...` values and `Cache-Control: no-store`, while `/api/oauth/account` returns the current `userid`, `username`, raw quota units, and active subscription snapshots (including `unlimited`/`plan_missing`, with `amount_remaining:null` for unlimited and `plan:null` when `plan_missing:true`) for polling, with eventually consistent `used_quota`.
...
- The native xAI channel supports Chat Completions, Responses, Responses Compact, Grok Imagine image generation/editing, and asynchronous Grok Imagine video generation/editing/extension. Video task handling lives in `relay/channel/task/xai/`; keep model names and request mappings synchronized with `docs/xai-channel.md`, `docs/video-generation-api.md`, and the official `https://docs.x.ai/llms.txt` catalog.
- MiniMax video tasks use the Hailuo task adaptor in `relay/channel/task/hailuo/`. `MiniMax-H3` routes to `/minimax/v2/video_generation` and `/minimax/v2/query/video_generation/{task_id}` with the v2 `content` schema, H3 resolution/duration/ratio validation, optional media references and callback options from `metadata`, and direct result URLs; legacy Hailuo models continue using the v1 endpoints. New API client configuration and request examples are documented in `docs/minimax-h3-new-api.md`.
