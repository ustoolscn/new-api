...
- The public `/service-status` page and `GET /api/service-status` endpoint default to hourly (24 buckets), with an optional daily (90 buckets) view, and display request counts, success rates, and average time-to-first-token for the whole site, individual models, and user groups. Request counts share the rankings display multiplier and stable jitter settings. TTFT aggregation stores sum/count/min/max coverage in the existing `perf_metrics` table; when complete extrema coverage and at least three samples are available, the public average removes one highest and one lowest sample before averaging, otherwise it falls back to the ordinary average. The current in-process bucket is included, no new table is added, and the endpoint remains rate-limited with capped public dimension output while reporting the full active counts.
