...
- The public `/service-status` page and `GET /api/service-status` endpoint display hourly (24 buckets) or daily (90 buckets) request counts and success rates for the whole site, individual models, and user groups. Statistics reuse the existing `perf_metrics` aggregation table plus the current in-process bucket, so this feature does not add a database table; the endpoint is rate-limited and caps public dimension output while reporting the full active counts.
