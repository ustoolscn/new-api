...
type TopUp struct {
	...
	InvoiceRequestId int     `json:"invoice_request_id" gorm:"index;not null;default:0"`
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		return createReferralCommissionTx(tx, topUp, quota)
	})
...
func RechargeEpay(tradeNo string, paymentMethod string, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		return createReferralCommissionTx(tx, &topUp, quotaToAdd)
	})
	if err != nil {
		return err
	}
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.PaymentProvider == PaymentProviderStripe {
			...
			quotaToAdd, quotaErr = topUpQuotaFromDecimal(decimal.NewFromInt(topUp.Amount))
		} else {
		...
		if err := createReferralCommissionTx(tx, topUp, quotaToAdd); err != nil {
			return err
		}
...
func RechargeWaffo(tradeNo string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		var quotaErr error
		quotaToAdd, quotaErr = topUpQuotaFromDecimal(
			decimal.NewFromInt(topUp.Amount).Mul(decimal.NewFromFloat(common.QuotaPerUnit)),
		)
		if quotaErr != nil {
			return quotaErr
