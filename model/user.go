...
type User struct {
	...
	SmsCode          string  `json:"sms_code" gorm:"-:all"`                                  // this field is only for phone verification, don't save it to database!
	...
	// Computed referral fields for admin user list (not persisted).
	InviteCount             int64                      `json:"invite_count" gorm:"-"`
	ReferralCommissionTotal int64                      `json:"referral_commission_total" gorm:"-"`
	ReferralRevenueTotal    int64                      `json:"referral_revenue_total" gorm:"-"`
...
func GetAllUsers(pageInfo *common.PageInfo, sortOptions ...UserSortOptions) (users []*User, total int64, err error) {
	...
	if err = fillUsersReferralStats(users); err != nil {
		return nil, 0, err
	}
...
// fillUsersReferralStats attaches invite counts and total referral revenue so the
// users table "Invite Info" matches the referrals tab (actual invitees + rewards + commissions).
func fillUsersReferralStats(users []*User) error {
	if len(users) == 0 {
		return nil
	}
	...
	for _, user := range users {
		...
		// Default revenue to registration invite rewards; commission is added below.
		user.ReferralRevenueTotal = int64(user.AffHistoryQuota)
	}
	if len(userIds) == 0 {
		return nil
	}
	...
	for _, aggregate := range inviteAggregates {
		if user, ok := userById[aggregate.InviterId]; ok {
			user.InviteCount = aggregate.InviteCount
		}
	}
	...
	if err := DB.Model(&ReferralCommission{}).
		Select("inviter_id, COALESCE(SUM(commission_quota), 0) AS total_quota").
		Where("inviter_id IN ?", userIds).
		Group("inviter_id").
	...
	for _, aggregate := range commissionAggregates {
		user, ok := userById[aggregate.InviterId]
		if !ok {
			continue
		}
		user.ReferralCommissionTotal = aggregate.TotalQuota
		user.ReferralRevenueTotal = int64(user.AffHistoryQuota) + aggregate.TotalQuota
	}
	return nil
}
