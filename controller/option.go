...
func UpdateOption(c *gin.Context) {
	...
	case "payment_setting.referral_commission_rate":
		rate, parseErr := strconv.ParseFloat(option.Value.(string), 64)
		if parseErr != nil || rate < 0 || rate > 100 {
			common.ApiErrorMsg(c, "邀请充值返佣比例必须在 0 到 100 之间")
			return
		}
	...
	case "theme.frontend":
		// Keep the legacy option for rolling upgrades, but never reactivate the
		// retired classic frontend. Older clients sending "classic" are treated
		// as a request for the only available frontend.
